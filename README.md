# RiskEstimator
Question 1 and 2 solutions and implementation could be found at
...
(2) Pulls market data from Yahoo Finance (or your favourite financial market data vendor) for the equity portfolio constructed above.
...
## Current Phase:
The current website is able to pull market data from Yahoo Finance, and portfolio weight assignment, as well as risk estimation function, is still under development.

### Current Web app Details:
1) [Stock Porfolio Section](http://127.0.0.1:8000/stocks/):
  - This tab is designed to let the user build up their own portfolio
...
2) [Dashboard Section](http://risk.zjtoronto.com:8000/dashboard/)
  - The website is able to retrieve stock historical data from Yahoo Fiance in the  Section
...
2. Download all dependencies in requirements.txt by running `pip install -r requirements.txt` in the command line.
Notice that the website is developed on MacBook with M1 chip, there may be some differences when installing on a Windows or Mac with Intel Chip.
...
4. Run the website locally by inputting` python manage.py runserver` on the terminal
...
Creates an interactive dashboard where you can select the dates and stocks to compare the share price history. The share price information is gathered from Yahoo through the Pandas Data Reader tool. By using Plotly Dash, the user can hover and interact with each stock, updating the tickers live.
